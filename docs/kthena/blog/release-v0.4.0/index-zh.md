...
此外，非常高兴地向大家同步：Kthena的`modelServing`和`Router`现已通过全面验证，能够良好地支持 **vLLM** 和 **SGLang** ，业界前沿的推理引擎。

为了帮助用户快速上手，Kthena v0.4.0 提供了完整的 [SGLang 部署示例和参考文档](https://github.com/volcano-sh/kthena/pull/883)。该指南可引导开发者毫无阻力地在 Kthena 中使用 SGLang。
