...
For new deployments, create `ModelServing`, `ModelServer`, and `ModelRoute` directly, with autoscaling policies as needed. The deprecated ModelBooster API creates these resources for existing users, as shown below. ModelBooster is deprecated in v1.1, with removal no earlier than v1.5; see the [deprecation details](../user-guide/model-deployment.md#modelbooster-deprecation).

```
ModelBooster  (deprecated deployment API)
...
| CRD | Purpose |
|---|---|
| **ModelBooster (deprecated)** | Retained for existing deployments. Captures model specs and cascades create/update/delete of downstream resources. |
...
- **Full platform** (default) — Both subcharts installed. Use `ModelServing` together with `ModelServer` and `ModelRoute` for end-to-end features such as PD-aware routing. Existing deployments using the deprecated `ModelBooster` API also require both subcharts.
