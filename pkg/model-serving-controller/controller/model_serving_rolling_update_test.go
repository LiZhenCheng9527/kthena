	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ms := &workloadv1alpha1.ModelServing{
				ObjectMeta: metav1.ObjectMeta{Name: "test-model-serving", Namespace: "default"},
			...
			result, err := controller.deleteOutdatedResourcesForRollingUpdate(
				context.Background(), ms, tt.maxScaleDown, tt.notRunningOutdatedGroups, tt.runningOutdatedGroups, "v1",
			)
...
func TestRolesToDeleteForRoleRollingUpdate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expected:         []roleToDelete{{roleName: "prefill", roleID: "prefill-3"}, {roleName: "prefill", roleID: "prefill-1"}},
			expectedOutdated: true,
		},
		{
